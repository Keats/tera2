...
pub(crate) enum Instruction {
    ...
    // Fused instructions (generated by peephole optimizer)
    /// Fused: LoadName + LoadAttr* (single push for entire path)
    /// path[0] is the variable name, path[1..] are attribute names
    LoadPath(Vec<String>),
    /// Fused: LoadName + LoadAttr* + WriteTop (bypasses stack entirely)
...
impl Chunk {
    ...
    /// This is a peephole optimization pass that reduces stack operations.
    pub(crate) fn optimize(&mut self) {
        ...
        while i < self.instructions.len() {
            ...
            if let (Instruction::LoadName(name), spans) = &self.instructions[i] {
                ...
                // Collect spans from all fused instructions for accurate error reporting
                ...
                while j < self.instructions.len() {
                    if let (Instruction::LoadAttr(attr), attr_spans) = &self.instructions[j] {
                        // Map the consumed LoadAttr to the same position as the fused instruction
                        index_map[j] = optimized.len();
                        path.push(attr.clone());
                        collected_spans.extend(attr_spans.iter().cloned());
                        j += 1;
                    } else {
                        break;
                    }
